...
export const Characters: FC = () => {
    ...
    return <>
        <SearchBar className="d-flex justify-content-center pb-5" placeHolder={"Search a character"} value={searchText}
                   onInputChange={(e: ChangeEvent<HTMLInputElement>) => handleInputChange(e)}
                   clearText={() => setSearchText("")}/>
        <div className="row row-cols-1 row-cols-xl-4 row-cols-md-3 row-cols-sm-2">
            {data?.characters?.results && data?.characters?.results?.length ?
        ...
        {data?.characters?.info?.pages > 0 &&
            <Pagination currentPage={currentPage} totalPages={data?.characters?.info?.pages}
